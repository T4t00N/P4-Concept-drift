...
def moco_loss(q, k, queue, tau=0.07):
    B = q.size(0)
...
# Model definition (updated to handle variable feature dimensions)
class SimCLRModel(nn.Module):
    def __init__(self, base_model, feature_dim, projection_dim=128):
        super(SimCLRModel, self).__init__()
        self.encoder = base_model
        self.projection_head = nn.Sequential(
            nn.Linear(feature_dim, 512),  # Input size matches encoder output
            nn.ReLU(),
            nn.Linear(512, projection_dim)  # Output size is 128
        )

    def forward(self, x):
        h = self.encoder(x)  # [batch_size, feature_dim]
...
def main():
    # Initialize WandB with updated model name
    wandb.init(project="MoCo", config={
        "learning_rate": 0.001,
        "epochs": 100,
        "batch_size": 256,
        "optimizer": "SGD",
        "model": "DINOv2 with MoCo",  # Updated to reflect DINOv2
        "queue_size": 65536,
        "momentum": 0.999,
        "tau": 0.07,
    })
    ...
    # Initialize models with DINOv2
    base_model_q = torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14', pretrained=True)
    base_model_k = torch.hub.load('facebookresearch/dinov2', 'dinov2_vits14', pretrained=True)
    feature_dim = 384  # DINOv2 ViT-S/14 output dimension
    query_model = SimCLRModel(base_model_q, feature_dim).to(device)
    key_model = SimCLRModel(base_model_k, feature_dim).to(device)
    key_model.load_state_dict(query_model.state_dict())  # Initialize key_model with query_model's weights
    ...
    # Optimizer for query_model only (unchanged)
    ...
    print("Starting MoCo training with DINOv2")
