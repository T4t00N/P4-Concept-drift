...
# Define the MoCo model (updated for DINOv2)
class MoCoModel(nn.Module):
    def __init__(self, base_model, feature_dim, projection_dim=128):
        super(MoCoModel, self).__init__()
        self.encoder = base_model  # DINOv2 model
        self.projection_head = nn.Sequential(
            nn.Linear(feature_dim, 512),  # Input size matches DINOv2 output (e.g., 384)
            nn.ReLU(),
            nn.Linear(512, projection_dim)  # Output size is 128
        )

    def forward(self, x):
        h = self.encoder(x)  # Feature representation (384-dim for DINOv2 ViT-S/14)
...
# Visualize clusters with t-SNE (unchanged)
...
def main():
    ...
    # Initialize the base model (DINOv2)
    ...
    model = MoCoModel(base_model, feature_dim).to(device)
