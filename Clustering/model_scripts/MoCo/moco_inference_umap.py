...
import umap  # Replaced TSNE with UMAP
...
# Save one random image per cluster (unchanged)
...
# Visualize clusters with UMAP (modified from t-SNE)
def visualize_clusters(features, cluster_labels, image_paths):
    """Visualize clusters using UMAP instead of t-SNE."""
    umap_model = umap.UMAP(n_components=2, random_state=42)
    features_2d = umap_model.fit_transform(features)

    df = pd.DataFrame({
        "UMAP 1": features_2d[:, 0],
        "UMAP 2": features_2d[:, 1],
    ...
    fig = px.scatter(
        ...
        title="Interactive UMAP Visualization of MoCo Image Clusters",
        labels={"color": "Cluster"}
    )

    fig.update_layout(legend_title_text="Cluster Label")
    fig.write_html("moco_interactive_umap_plot.html")
    print("Interactive UMAP plot saved as moco_interactive_umap_plot.html")

# Main function (updated)
def main():
    # Parameters to adjust
    checkpoint_path = "moco_checkpoints/moco_epoch_100.pt"  # Path to MoCo checkpoint
    ...
    output_html = "moco_interactive_umap_plot.html"  # Updated HTML visualization name
