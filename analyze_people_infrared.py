...
def load_metadata(dataset_root):
    ...
    for json_file in json_files:
        ...
        # Collect annotations for people (category_id = 1)
...
for img in images_metadata:
    ...
    # Parse datetime from file path (e.g., frames/20200514/clip_10_1744/image_0106.jpg)
    date_str = file_name.split("/")[1]  # 20200514
    time_str = file_name.split("/")[2].split("_")[-1]  # 1744
    try:
...
# Add bounding boxes for people
for ann in people_annotations:
    image_id = ann["image_id"]
    if image_id in image_info:
        image_info[image_id]["people_bboxes"].append(ann["bbox"])  # [x, y, w, h]
...
print("Sample of loaded image data:")
for entry in image_data[:5]:
    print(f"  - Path: {entry['path']}, Datetime: {entry['datetime']}, People BBoxes: {entry['people_bboxes']}")


print("Defining background region (pavement)...")
background_roi = (100, 50, 284, 200)  # (x, y, w, h) for pavement
...
def compute_people_intensities(image_path, people_bboxes, background_roi):
    ...
    # Compute average intensity for people regions
    people_intensities = []
    ...
    # Average across all people in the image
...
for i, entry in enumerate(image_data, 1):
    if not entry["people_bboxes"]:  # Skip images with no people
        continue
...
print("Generating Plot 4: Scatter Plot of People Intensity vs. Background Intensity (Colored by Time of Day)...")
