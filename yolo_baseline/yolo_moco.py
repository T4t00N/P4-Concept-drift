...
def train(args, params):
    ...
    dataset = Dataset(filenames, args.input_size, params, augment=False, cluster_label=args.cluster_label)
    ...
    if args.use_original_val:
        ...
        with open(f'{path}/val_name.txt') as reader:
            for fp in reader.readlines():
                ...
                img_file = fp.strip()  # current line keeps only the filename
                if not os.path.isabs(img_file):  # if it’s just “20200802….jpg”
                    img_file = os.path.join(path, "images", img_file)  # prepend the dataset root
...
def main():
    ...
    parser.add_argument('--val-cluster-label', type=int, default=1,
                        help='cluster to use for validation')
